    ...
    for r in range(N):         # r is the current row
        for c in range(N):     # c is the current column
            ...
            # lower left corner (3 neighbors)
                ...
                total = current[r-1][c] + current[r][c+1] + current[r-1][c+1]
            
            # lower right corner (3 neighbors)
            elif r == N-1 and c == N-1:
                total = current[r-1][c] + current[r][c-1] + current[r-1][c-1]
            
            else: #middle section here
                total = current[r-1][c-1] + current[r-1][c] + current[r-1][c+1] + current[r][c-1] + current[r][c+1] + current[r+1][c-1] + current[r+1][c] + current[r+1][c+1]
            ...
            # or ALIVE
            if current[r][c] == ALIVE and (total == 2 or total == 3):
                next_gen[r][c] = ALIVE
            elif current[r][c] == DEAD and total == 3:
                next_gen[r][c] = ALIVE
            else:
                next_gen[r][c] = DEAD
